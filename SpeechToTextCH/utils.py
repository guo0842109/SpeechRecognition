...
class SpeechLoader():
	...
	def __init__(self, wav_path=None, label_file=None, batch_size=1, n_mfcc=60, encoding='utf-8', is_test = False):
		self.batch_size = batch_size
		self.encoding = encoding
		self.n_mfcc = n_mfcc

		# path setting
		data_dir = os.path.join('data', 'cache', 'mfcc'+str(n_mfcc))
		# data cache
		...
		if(is_test == True):
			self.vocab_size = 2666
			self.wav_max_len = 673
			self.label_max_len = 75
	...
	def preprocess(self, wav_path, label_file, wavs_file, vocab_file, mfcc_tensor, label_tensor):
		...
		print("Sample number:", len(wav_files)) #样本数
		...
		print("Sample cont:", cnt)
		print("Max wave len", self.wav_max_len)
		...
		print("Max label len", self.label_max_len)
	...
	def load_preprocessed(self, vocab_file, mfcc_tensor, label_tensor):
		with open(vocab_file, 'rb') as f:
			self.wordmap = cPickle.load(f) 
		self.vocab_size = len(self.wordmap)
		print("Voc len",self.vocab_size)

		with open(mfcc_tensor, 'rb') as f:
			self.mfcc_tensor = cPickle.load(f)
		self.wav_max_len = max(len(mfcc) for mfcc in self.mfcc_tensor)
		print("Max wav len", self.wav_max_len)
